...
public class CodDepartamentosDaoImpl implements CodDepartamentosDao {
	...
	private static String INS_DTO = "Insert into COD_DEPARTAMENTOS (ID_DEPARTAMENTO, DEPARTAMENTO) Values(UPPER(?),UPPER(?))"; 
	private static String DEL_DTO = "Delete from COD_DEPARTAMENTOS where ID_DEPARTAMENTO= UPPER(?)";
	private static String UPD_DTO = "Update COD_DEPARTAMENTOS set DEPARTAMENTO = UPPER(?) where ID_DEPARTAMENTO= ?";
	
	private static String TODOS = "Select * from COD_DEPARTAMENTOS order by DEPARTAMENTO"; 
	private static String TODOS_POR_ID = "Select * from COD_DEPARTAMENTOS Where ID_DEPARTAMENTO= UPPER (?)"; 
	private static String TODOS_POR_DEPARTAMENTO = "Select * from COD_DEPARTAMENTOS Where DEPARTAMENTO= UPPER (?)";
	private static String TODOS_POR_DEPARTAMENTO_LIKE = "Select * from COD_DEPARTAMENTOS Where DEPARTAMENTO LIKE UPPER( '%' || ? || '%')";
	...
	public List<CodDepartamento> obtenerCodDepartamento(String descDepartamento) {
		...
		try {
			...
			while ( bd.getResultSet().next()) {
				...
				dto.setIdCodDepartamento( bd.getResultSet().getString(1));
